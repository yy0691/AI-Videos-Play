...
### Fix Record - 2025-11-19

-   **Problem Description:** 配置 FFmpeg CDN 后仍然失败，浏览器报错 404 和 CORS 错误：
    - `GET https://cdn.jsdelivr.net/npm/@ffmpeg/core@0.12.6/dist/umd/ffmpeg-core.worker.js 404 (Not Found)`
    - `Access to fetch at 'https://unpkg.com/@ffmpeg/core@0.12.6/dist/umd/ffmpeg-core.worker.js' has been blocked by CORS policy`
-   **Root Cause:** 
    1. **代码错误**：`videoSplitterService.ts` 尝试加载3个文件（ffmpeg-core.js、ffmpeg-core.wasm、ffmpeg-core.worker.js）
    2. **CDN实际情况**：@ffmpeg/core 0.12.x 单线程版本只有2个文件（.js 和 .wasm），**不存在** worker.js 文件
    3. **版本问题**：之前推荐的 0.12.6 版本不是最新版，应使用 0.12.10（与项目中的 @ffmpeg/ffmpeg 版本匹配）
    4. 通过浏览器访问 unpkg.com CDN 确认：`dist/umd/` 和 `dist/esm/` 目录都只有2个文件
-   **Solution:** 
    1. **修复代码** (`services/videoSplitterService.ts`)：
       - 移除对 `ffmpeg-core.worker.js` 的加载尝试
       - 只加载2个必需文件：`ffmpeg-core.js` 和 `ffmpeg-core.wasm`
       - 添加注释说明 0.12.x 单线程版本不需要 worker.js
    2. **更新所有文档中的版本号**：
       - 将所有 `@ffmpeg/core@0.12.6` 改为 `@ffmpeg/core@0.12.10`
       - 更新 `@docs/FFMPEG_CDN_SETUP.md` 中的所有配置示例
       - 更新 `README.md` 中的推荐配置
    3. **完善技术文档**：
       - 添加"CDN 文件列表"章节，明确说明只需2个文件
       - 更新"版本对应关系"表格
-   **Impact Scope:**
    -   `services/videoSplitterService.ts` (第162-172行，移除 worker.js 加载)
    -   `@docs/FFMPEG_CDN_SETUP.md` (所有版本号从 0.12.6 → 0.12.10，添加技术说明)
    -   `README.md` (更新推荐的 CDN URL)
...
**技术细节**：
- @ffmpeg/core 0.12.x 单线程版本将 worker 逻辑内嵌在 ffmpeg-core.js 中
- 只需加载2个文件：ffmpeg-core.js (112 KB) + ffmpeg-core.wasm (32.2 MB)
- 通过 FFmpeg.load({ coreURL, wasmURL }) 即可完成初始化
...
-   **Root Cause:** 
    1. FFmpeg.wasm 在浏览器中需要从CDN加载核心文件
    ...
    5. 默认配置 `DEFAULT_BASE_URLS` 被有意留空
-   **Solution:** 
...
**注**：此次修复提供了配置文档，但推荐的CDN URL有误，在下一次修复中更正。
...
-   **Problem Description:** 
    1. 视频跳转到指定时间点时，字幕区域未能自动滚动到对应的字幕位置
    2. 字幕已生成完整，但关键时刻和见解未生成，且见解面板显示"无法生成"的错误信息
-   **Root Cause:** 
    1. **字幕滚动定位问题**：
       - `handleSeekTo` 函数中虽然调用了 `scrollToActiveSegment()`，但存在时序问题
       - 当视频跳转时，`activeSegmentIndex` 可能还没有更新（因为它是根据 `currentTime` 计算的）
       - 如果用户点击跳转时没有提供 `segmentIndex`，需要根据时间计算对应的字幕索引
       - 滚动延迟设置不够，导致 DOM 和 ref 更新不完整
    2. **关键时刻和见解生成问题**：
       - 生成过程中缺少详细的错误处理和日志记录
       - 当生成失败时，错误信息不够清晰，用户无法了解具体原因
       - 缺少对生成结果的验证，可能导致空结果被保存
-   **Solution:** 
    1. **修复字幕滚动定位**：
       - 在 `handleSeekTo` 函数中，如果没有提供 `segmentIndex`，根据跳转时间自动计算对应的字幕索引
       - 立即设置 `clickedSegmentIndex`，确保 `activeSegmentIndex` 正确计算
       - 优化滚动时机，使用多重延迟（50ms + requestAnimationFrame + 50ms）确保 DOM 和 ref 都已更新
       - 确保无论视频是否播放，跳转时都能正确滚动到对应位置
    2. **改进关键时刻和见解生成**：
       - 在 `handleGenerateInsights` 中添加详细的调试日志，记录字幕数据、视频哈希、现有分析等信息
       - 在 `prepareAnalysisPayload` 中添加日志，记录字幕数据准备过程
       - 在 `generateResilientInsights` 中为每个分析类型添加详细的日志记录
       - 添加结果验证，确保生成的结果不为空
       - 改进错误处理，提供更清晰的错误信息
       - 使用 toast 通知替代 alert，提供更好的用户体验
       - 添加成功和失败提示，让用户了解生成状态
-   **Impact Scope:** 
    -   `components/VideoDetail.tsx`（修复 `handleSeekTo` 函数，改进 `handleGenerateInsights` 函数）
    -   `services/videoProcessingService.ts`（改进 `prepareAnalysisPayload` 和 `generateResilientInsights` 函数，添加详细日志和错误处理）

---

### Fix Record - 2025-01-27 (续)

-   **Problem Description:** Deepgram 生成字幕时无法知道需要多长时间，用户无法预估等待时间
-   **Root Cause:** 
    - Deepgram API 只提供进度百分比回调，不提供时间估算
    - 代码中只有超时时间设置（用于防止请求超时），但没有实际处理时间估算
    - 现有的 `getProcessingEstimate` 函数只适用于 Gemini 处理，不适用于 Deepgram
-   **Solution:** 
    1. **添加 Deepgram 处理时间估算函数**：
       - 在 `deepgramService.ts` 中新增 `estimateDeepgramProcessingTime` 函数
       - 估算包括三个部分：
         - **音频提取时间**（如果需要）：视频时长的 5-15%
         - **上传时间**：基于文件大小和网络速度（10-50 MB/s）
         - **处理时间**：基于音频时长，Deepgram 通常比实时快 3 倍（1 分钟音频需要 20 秒处理）
         - **缓冲时间**：10-20 秒（网络延迟、API 响应等）
       - 返回最小和最大估算时间（秒）
    2. **在 VideoDetail 组件中集成估算**：
       - 检测是否使用 Deepgram（通过 `isDeepgramAvailable`）
       - 如果使用 Deepgram，调用 `estimateDeepgramProcessingTime` 获取估算
       - 根据文件大小判断是否需要音频提取（>4MB 需要提取）
       - 格式化时间显示（秒/分钟）
       - 在用户确认对话框和日志中显示估算时间
-   **Impact Scope:** 
    -   `services/deepgramService.ts`（添加 `estimateDeepgramProcessingTime` 函数）
    -   `components/VideoDetail.tsx`（集成 Deepgram 时间估算，在生成前显示估算时间）

**估算公式说明**：
- **总时间 = 音频提取时间（如果需要）+ 上传时间 + 处理时间 + 缓冲时间**
- **上传时间**：文件大小 ÷ 上传速度（10-50 MB/s）
- **处理时间**：音频时长 ÷ 3（Deepgram 通常 3 倍实时速度）
- **音频提取时间**：视频时长 × 5-15%（如果需要提取）

**示例**：
- 100MB 视频，10 分钟时长，需要音频提取：
  - 音频提取：10分钟 × 10% = 1分钟
  - 上传：100MB ÷ 30MB/s = 3.3秒（最小）到 10秒（最大）
  - 处理：10分钟 ÷ 3 = 3.3分钟
  - 缓冲：10-20秒
  - **总计**：约 4.5-5 分钟
...
-   **Problem Description:** 翻译字幕的时候，不应该又重新生成见解和关键时刻
-   **Root Cause:** 
    - 翻译字幕时，虽然不会直接调用 `handleGenerateSubtitles`（该函数会在字幕生成完成后自动生成见解）
    - 但翻译完成后调用 `onSubtitlesChange(video.id)` 可能会在某些情况下触发数据重新加载
    - 虽然代码中已经有注释说明"翻译操作不应该触发见解生成"，但没有明确的保护机制
    - 翻译只是更新了字幕的 `translatedText` 字段，并没有改变原始字幕内容（`text` 字段）
    - 见解和关键时刻应该基于原始字幕生成，翻译后不需要重新生成
-   **Solution:** 
    - 在 `handleTranslateSubtitles` 函数中添加明确的日志说明，确保翻译操作不会触发见解生成
    - 明确注释说明翻译操作与字幕生成操作的区别：
      - **字幕生成**：创建新的字幕内容（`text` 字段），完成后如果没有见解则自动生成
      - **字幕翻译**：只更新翻译文本（`translatedText` 字段），不应该触发见解重新生成
    - 通过 `isTranslationFromUser` 标志区分翻译操作和字幕生成操作
    - 添加日志记录，便于调试和确认翻译操作不会触发见解生成
-   **Impact Scope:** 
    -   `components/VideoDetail.tsx`（改进 `handleTranslateSubtitles` 函数，添加明确的注释和日志）

**技术说明**：
- 翻译字幕时，`handleTranslateSubtitles` 函数只更新字幕的 `translatedText` 字段
- 原始字幕内容（`text` 字段）保持不变，因此见解和关键时刻不需要重新生成
- `handleGenerateSubtitles` 函数中的自动生成见解逻辑只在字幕生成时执行，不会在翻译时触发

---

### Fix Record - 2025-01-27 (续3)

-   **Problem Description:** 确认 Linux.do 用户详情接口 URL 为 `https://connect.linux.do/api/user`
-   **Root Cause:** 
    - 代码中已使用正确的 URL，但需要确认所有地方都使用了正确的端点
    - 需要添加注释说明这是正确的用户信息端点
-   **Solution:** 
    - 确认 `services/linuxDoAuthService.ts` 中的 `LINUXDO_USER_INFO_URL` 常量已设置为 `https://connect.linux.do/api/user`
    - 添加注释说明这是正确的用户信息端点
    - 确认文档中也记录了正确的端点 URL
-   **Impact Scope:** 
    -   `services/linuxDoAuthService.ts`（添加注释确认正确的用户信息端点）
...
-   **Problem Description:** 确认 Linux.do OAuth 配置与 Supabase 中的 Slack OIDC 配置不会冲突
-   **Root Cause:** 
    - 用户担心在 Supabase Dashboard 中配置的 Slack OIDC 服务会影响 Linux.do 登录
    - 需要明确说明两者的区别和独立性
-   **Solution:** 
    - 在 `getLinuxDoConfig` 函数中添加详细注释，说明 Linux.do OAuth 配置与 Supabase 内置 OAuth provider 的区别
    - 明确说明：
      1. **Supabase 的 Slack OIDC**：在 Supabase Dashboard → Authentication → Providers 中配置，用于 Supabase Auth 系统（用户通过 Slack 登录 Supabase）
      2. **Linux.do OAuth**：存储在数据库的 `oauth_config` 表中，使用 `provider='linuxdo'` 区分，是应用层面的自定义 OAuth 实现
      3. **不会冲突的原因**：
         - 查询时使用 `.eq('provider', 'linuxdo')` 过滤，只获取 Linux.do 的配置
         - 不同的存储位置和用途
         - 完全独立的认证流程
-   **Impact Scope:** 
    -   `services/linuxDoAuthService.ts`（添加注释说明配置独立性）

**配置说明**：
- **Supabase Dashboard 配置**（如 Slack OIDC）：用于 Supabase Auth 系统，用户可以通过 Slack 登录 Supabase 账户
- **数据库 oauth_config 表配置**（Linux.do）：用于应用层面的自定义 OAuth 实现，与 Supabase Auth 完全独立
- **两者可以共存**：可以在 Supabase 中同时配置 Slack OIDC（用于 Supabase Auth）和在 oauth_config 表中配置 Linux.do（用于应用 OAuth）

---

### Fix Record - 2025-01-27 (续5)

-   **Problem Description:** Linux.do OAuth 登录出现 `invalid_request` 错误，诊断信息显示 `redirect_uri` 可能不匹配
-   **Root Cause:** 
    - 代码中 `buildLinuxDoAuthUrl` 函数会自动移除 `redirect_uri` 的尾部斜杠
    - 但 Linux.do OAuth 对 `redirect_uri` 的匹配非常严格，必须与注册时配置的完全一致
    - 如果 Linux.do 应用中配置的回调 URL 是 `https://insight.luoyuanai.cn/`（有斜杠），但代码发送的是 `https://insight.luoyuanai.cn`（无斜杠），就会导致 `invalid_request` 错误
    - 反之亦然：如果配置的是无斜杠的，但代码发送的是有斜杠的，也会失败
-   **Solution:** 
    1. **移除自动移除尾部斜杠的逻辑**：
       - 修改 `buildLinuxDoAuthUrl` 函数，不再自动移除尾部斜杠
       - 保持原始 `redirect_uri` 值，确保与 Linux.do 应用中配置的完全一致
    2. **添加详细的诊断信息**：
       - 在构建 OAuth URL 时输出详细的诊断信息
       - 包括原始 URL、是否有尾部斜杠、当前 origin 和 pathname
       - 提供明确的提示，说明需要与 Linux.do 应用中的配置完全匹配
    3. **更新文档**：
       - 在 `LINUXDO_OAUTH_SETUP.md` 中添加详细的 `redirect_uri` 匹配规则说明
       - 明确说明必须包括尾部斜杠（如果有的话）
       - 提供示例和注意事项
    4. **添加警告提示**：
       - 在 `AccountPanel.tsx` 和 `AuthModal.tsx` 中添加警告日志
       - 提醒用户确保 `redirect_uri` 与 Linux.do 应用中的配置完全一致
-   **Impact Scope:** 
    -   `services/linuxDoAuthService.ts`（移除自动移除尾部斜杠的逻辑，添加详细诊断信息）
    -   `components/AccountPanel.tsx`（添加警告提示）
    -   `components/AuthModal.tsx`（添加警告提示）
    -   `@docs/LINUXDO_OAUTH_SETUP.md`（添加详细的 redirect_uri 匹配规则说明）

**解决方案**：
1. **检查 Linux.do 应用中的回调 URL 配置**：
   - 登录 Linux.do 开发者控制台
   - 查看 OAuth 应用的回调 URL 配置
   - 确认是 `https://insight.luoyuanai.cn/`（有斜杠）还是 `https://insight.luoyuanai.cn`（无斜杠）

2. **确保代码中的 redirect_uri 与配置完全一致**：
   - 打开浏览器控制台（F12）
   - 点击 Linux.do 登录按钮
   - 查看 "redirect_uri 诊断信息" 日志
   - 确认显示的 URL 与 Linux.do 应用中配置的完全一致（包括尾部斜杠）

3. **如果仍然不匹配**：
   - 修改 Linux.do 应用中的回调 URL 配置，使其与代码中使用的完全一致
   - 或者修改代码，使其与 Linux.do 应用中的配置完全一致

**重要提示**：
- OAuth 对 `redirect_uri` 的匹配非常严格，必须完全一致
- 包括：协议（http/https）、域名、路径、尾部斜杠等所有细节
- 建议在 Linux.do 应用中配置时，使用与代码中完全相同的格式

**排查步骤**：
1. 打开浏览器控制台（F12）
2. 点击 Linux.do 登录按钮
3. 查看 "🔍 OAuth 请求诊断信息" 日志
4. 复制日志中的 `redirect_uri` 值
5. 登录 Linux.do 开发者控制台
6. 检查 OAuth 应用的回调 URL 配置
7. 确保回调 URL 与日志中的 `redirect_uri` 完全一致（包括尾部斜杠）
8. 如果不一致，修改 Linux.do 应用中的回调 URL 配置

**常见问题**：
- 如果 Linux.do 应用中配置的是 `https://insight.luoyuanai.cn/`（有斜杠），但代码发送的是 `https://insight.luoyuanai.cn`（无斜杠）→ 修改 Linux.do 应用中的配置为无斜杠
- 如果 Linux.do 应用中配置的是 `https://insight.luoyuanai.cn`（无斜杠），但代码发送的是 `https://insight.luoyuanai.cn/`（有斜杠）→ 修改代码或 Linux.do 应用中的配置，使其一致

---

### Fix Record - 2025-11-22

-   **Problem Description:** Linux.do 登录功能中，前端代码需要手动构建 redirect_uri，而 Google/GitHub 登录不需要。代码重复且不一致，AuthModal.tsx 和 AccountPanel.tsx 中都有构建逻辑但实现略有不同。
-   **Root Cause:** 
    1. Linux.do OAuth 是自定义流程，需要手动构建 redirect_uri
    2. 前端代码在多个地方重复实现了 redirect_uri 的构建逻辑
    3. AuthModal.tsx 和 AccountPanel.tsx 中的实现不一致（一个移除了尾部斜杠，一个注释掉了）
    4. 与 Google/GitHub 登录的简洁性不一致（它们只需要调用 `authService.signInWithGoogle()` 等）
-   **Solution:** 
    1. **修改 `buildLinuxDoAuthUrl` 函数**：
       - 将 `redirectUri` 参数改为可选（`redirectUri?: string`）
       - 如果未提供 `redirectUri`，自动从 `window.location` 构建
       - 统一处理 redirect_uri 的规范化逻辑（移除尾部斜杠等）
       - 添加 JSDoc 注释说明，强调与 Google/GitHub 登录的一致性
    2. **简化 `AuthModal.tsx`**：
       - 移除 `handleLinuxDoSignIn` 中手动构建 redirect_uri 的代码
       - 直接调用 `buildLinuxDoAuthUrl()` 而不传参数
       - 代码从 30+ 行简化为 10+ 行
    3. **简化 `AccountPanel.tsx`**：
       - 移除 `handleLinuxDoLogin` 中手动构建 redirect_uri 的代码
       - 直接调用 `buildLinuxDoAuthUrl()` 而不传参数
       - 代码从 30+ 行简化为 10+ 行
-   **Impact Scope:** 
    -   `services/linuxDoAuthService.ts`（修改 `buildLinuxDoAuthUrl` 函数签名和实现，使 redirect_uri 参数可选）
    -   `components/AuthModal.tsx`（简化 `handleLinuxDoSignIn` 函数，移除手动构建 redirect_uri 的代码）
    -   `components/AccountPanel.tsx`（简化 `handleLinuxDoLogin` 函数，移除手动构建 redirect_uri 的代码）

**改进效果**：
- **代码一致性**：Linux.do 登录现在与 Google/GitHub 登录保持一致的简洁风格
- **代码复用**：redirect_uri 构建逻辑统一在 `linuxDoAuthService.ts` 中处理
- **维护性提升**：前端代码更简洁，减少重复代码
- **向后兼容**：`buildLinuxDoAuthUrl` 仍然支持手动传入 `redirectUri` 参数（如果需要）
...
// 修复后（AuthModal.tsx）：
const authUrl = await buildLinuxDoAuthUrl(); // 自动从 window.location 构建
```
